# Copied from the hfilesize library
import math
import re
...
class FileSize(int):
    ...
    def format(self, base=1024, exponent=None, float_fmt=".2f", size_fmt=Format.casing):
        ...
        if self % (base**exponent) == 0:
            ...
            size = self / (base**exponent)
        # format final result
        return "{size:{float_fmt}}{suffix}".format(
            size=size, float_fmt=float_fmt, suffix=suffix
        )
    ...
    def __format__(self, fmt):
        ...
        float_res = self.format(
            base=base,
            exponent=exponent,
            float_fmt=float_fmt_matches.group(),
            size_fmt=size_fmt,
        )
